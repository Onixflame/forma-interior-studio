...
test('package choices reflect area, property and condition from the previous step',()=>{
 const input={...defaults,area:88,property:'house' as const,condition:'extensive' as const};
 assert.deepEqual(calculate({...input,package:'Essential'}),{low:72600,high:96800});
 assert.deepEqual(calculate({...input,package:'Complete'}),{low:108900,high:145200});
 assert.deepEqual(calculate({...input,package:'Signature'}),{low:169400,high:217800});
});
