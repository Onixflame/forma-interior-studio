...
test('gallery previews, arrows, escape and focus restoration',async({page})=>{await page.goto('/en/projects/the-oak-residence/');const previews=page.locator('.project-gallery-thumb');await expect(previews).toHaveCount(2);await previews.nth(1).click();await expect(page.locator('.project-gallery-caption')).toContainText('2 / 2');const open=page.getByRole('button',{name:'Open image gallery: The details'});await open.click();await expect(page.locator('dialog.lightbox')).toBeVisible();await page.keyboard.press('ArrowLeft');await expect(page.locator('.lightbox-bottom p')).toContainText('1 / 2');await page.keyboard.press('Tab');expect(await page.evaluate(()=>!!document.activeElement?.closest('dialog'))).toBeTruthy();await page.keyboard.press('Escape');await expect(page.locator('dialog.lightbox')).not.toBeVisible();await expect(open).toBeFocused();});
...
test('project gallery is a single stage with scalable horizontal previews',async({page})=>{
 await page.goto('/ru/projects/the-oak-residence/');
 await expect(page.locator('.project-gallery-stage')).toHaveCount(1);
 await expect(page.locator('.project-gallery-thumbs')).toHaveCount(1);
 await expect(page.locator('.project-gallery-thumb')).toHaveCount(2);
 const boxes=await page.locator('.project-gallery-thumb').evaluateAll(items=>items.map(el=>({left:(el as HTMLElement).offsetLeft,width:(el as HTMLElement).offsetWidth})));
